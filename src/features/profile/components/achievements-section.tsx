...
/**
 * Sección de logros — vive dentro de `/app/perfil` (con `id="logros"` para
 * que `/app/logros` pueda seguir enlazando ahí como redirect permanente).
 */
export function AchievementsSection({ stats }: { stats: AchievementStats }) {
  const unlocked = BADGES.filter((b) => b.unlockedWhen(stats));
  const locked = BADGES.filter((b) => !b.unlockedWhen(stats));
  const percent = Math.round((unlocked.length / BADGES.length) * 100);

  return (
    <section id="logros" className="scroll-mt-20">
      <SectionRule trailing={`${unlocked.length} de ${BADGES.length}`}>Logros</SectionRule>
      <p className="mt-3 max-w-[56ch] text-[15px] leading-relaxed text-muted-foreground">
        {unlocked.length === 0
          ? "Cada lección y cada reto que completes desbloquea un logro."
          : `Llevas ${unlocked.length} de ${BADGES.length} ${pluralize(BADGES.length, "logro", "logros")}.`}
      </p>
      ...
      <ReadoutBar className="mt-6">
        <Readout label="XP totales" value={<AnimatedNumber value={stats.totalXp} />} />
        <Readout
          label="Mejor racha"
        ...
        <Readout
          ...
          value={<AnimatedNumber value={stats.exercisesPassed} />}
        />
      </ReadoutBar>

      {unlocked.length === 0 ? (
        <div className="mt-6 rounded-[var(--radius-lg)] border border-primary/25 bg-primary-tint p-5 sm:p-6">
          ...
          <Button asChild size="lg" className="mt-4 max-sm:w-full">
            <Link href="/app">
              Empezar una lección
              <ArrowRight />
            </Link>
      ...
      {locked.length > 0 ? (
        <div className="mt-6">
          <p className="mb-3 text-[13px] font-bold uppercase tracking-[0.06em] text-subtle-foreground">
...
function BadgeList({ badges, unlocked }: { badges: BadgeDef[]; unlocked: boolean }) {
  return (
    <ul
      data-stagger
          ...
          >
            <span
              ...
              className={cn(
                ...
                unlocked ? TONE_MARK[badge.tone] : "bg-surface-2 text-subtle-foreground",
              )}
            >
              {unlocked ? <Icon className="size-5" /> : <Lock className="size-[18px]" />}
            </span>
