...
function EmptyAmigosHero({ username }: { username: string }) {
  return (
    <div className="relative overflow-hidden rounded-[var(--radius-xl)] border border-primary/20 bg-card p-6 shadow-[var(--shadow-sm)] sm:p-8">
      <div
        aria-hidden
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Tu handle es <span className="font-mono text-foreground">@{username}</span>.
          </p>
        </div>
      </div>
    </div>
