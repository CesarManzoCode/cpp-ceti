...
export default async function CourseHomePage({ params }: PageProps) {
  ...
  // El curso sale de la URL. Todo lo que sigue (unidades, progreso, próxima
  // lección, práctica) queda acotado a ESTE curso: nada cruza cursos.
  const course = await getCourseBySlug(courseSlug);
  if (!course) notFound();
      ...
      <div className="mt-5 grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px] xl:gap-10">
        <div className="min-w-0">
          {nextLesson ? (
            ...
            <AllDonePanel courseSlug={course.slug} />
          ) : (
...
function ContinuePanel({
  ...
  const href = `/app/c/${courseSlug}/u/${next.unitSlug}/${next.lessonSlug}`;
